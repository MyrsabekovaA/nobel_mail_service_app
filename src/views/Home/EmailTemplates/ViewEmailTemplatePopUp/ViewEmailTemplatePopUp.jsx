...
function ViewEmailTemplatePopUp() {
    ...
    return <>
        {templateNumber>0&&<button onClick={()=>setTemplateNumber(templateNumber-1)}>{"<"}</button>}
        <div dangerouslySetInnerHTML={{ __html: template }}></div>
        {templateNumber+1<SelectedTemplates.length&&<button onClick={()=>setTemplateNumber(templateNumber+1)}>{">"}</button>}
    </>
}
