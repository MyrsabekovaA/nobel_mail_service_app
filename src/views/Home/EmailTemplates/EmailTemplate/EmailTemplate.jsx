...
const EmailTemplate = (props) => {
    ...
    const emailTemplates = useSelector(state=>state.emailTemplates.selectedTemplates)
    console.log(emailTemplates)
    //counting if checked
    let checked = emailTemplates.some(template=>template.id===id)||null
    const handleChange = (e) => {
        if(e.target.checked === true) {
            dispatch(emailTemplatesActions.emailTemplatesSelectedPush({id, name, googleDriveFileId}))
        }
        else {
            console.log(1)
    ...
    return <tr>
            ...
            <td className="email-templates__table-cell"><a href={`https://docs.google.com/document/d/${googleDriveFileId}/edit`}>See in google docs</a></td>
        </tr>
}
